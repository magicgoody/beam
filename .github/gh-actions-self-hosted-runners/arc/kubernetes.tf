...
resource "kubectl_manifest" "arc_deployment" {
  yaml_body = templatefile("config/arc_deployment.tpl", { repository = "${var.organization}/${var.repository}" })
  override_namespace = "arc"
  depends_on = [ helm_release.arc ]
}
resource "kubectl_manifest" "arc_autoscaler" {
  yaml_body = templatefile("config/arc_autoscaler.tpl", { min_runners = var.min_main_replicas, max_runners = var.max_main_replicas })
  override_namespace = "arc"
  depends_on = [ helm_release.arc ]
}
