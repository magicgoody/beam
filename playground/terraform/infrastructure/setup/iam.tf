...
resource "google_project_iam_member" "terraform_service_account_roles" {
  for_each = toset([
   "roles/container.admin", "roles/artifactregistry.reader", "roles/datastore.owner", "roles/redis.admin", google_project_iam_custom_role.playground_datastore_role.role_id,
  ])
  role    = each.value
