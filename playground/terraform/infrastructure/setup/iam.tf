...
resource "google_project_iam_member" "terraform_service_account_roles" {
  for_each = toset([
     "roles/container.developer", "roles/artifactregistry.reader", "roles/datastore.viewer", "roles/datastore.user", "roles/redis.serviceAgent", "roles/redis.viewer", google_project_iam_custom_role.playground_datastore_role.role_id ,
  ])
  role    = each.value
